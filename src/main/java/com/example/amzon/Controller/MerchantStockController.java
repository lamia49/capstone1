...
public class MerchantStockController {
    ...
    public ResponseEntity updste(@PathVariable int id, @RequestBody @Valid MerchantStock merchantStock, Errors error) {
        if (error.hasErrors()) {
            String massege = error.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(massege);
        }

        boolean isFounf = merchantStockService.update(id, merchantStock);
        if (isFounf) {
